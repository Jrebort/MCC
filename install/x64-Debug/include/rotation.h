...
template<typename T>
inline T DotProductWithPointFix(const T x[3], const double y[3]) {
    return (x[0] * y[0] + x[1] * y[1] + x[2] * y[2]);
}
...
template<typename T>
inline void CrossProductWithPointFix(const T x[3], const double y[3], T result[3]) {
    result[0] = x[1] * y[2] - x[2] * y[1];
    result[1] = x[2] * y[0] - x[0] * y[2];
    result[2] = x[0] * y[1] - x[1] * y[0];
}
...
template<typename T>
inline void AngleAxisRotatePointWithPointFix(const T angle_axis[3], const double pt[3], T result[3]) {
    ...
    if (theta2 > T(std::numeric_limits<double>::epsilon())) {
        ...
        /*const T w_cross_pt[3] = { w[1] * pt[2] - w[2] * pt[1],
                                  ...
                                  w[0] * pt[1] - w[1] * pt[0] };*/
        T w_cross_pt[3];
        CrossProductWithPointFix(w, pt, w_cross_pt);

        const T tmp = DotProductWithPointFix(w, pt) * (T(1.0) - costheta);
        ...
        T w_cross_pt[3];
        CrossProductWithPointFix(angle_axis, pt, w_cross_pt);
